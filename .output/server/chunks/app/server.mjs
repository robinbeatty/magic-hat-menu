import { hasInjectionContext, getCurrentInstance, version, unref, inject, defineComponent, h, computed, ref, provide, shallowReactive, Suspense, nextTick, Transition, useSSRContext, mergeProps, withCtx, createVNode, createApp, effectScope, reactive, defineAsyncComponent, onErrorCaptured, onServerPrefetch, resolveDynamicComponent, toRef, shallowRef, isReadonly, isRef, isShallow, isReactive, toRaw } from 'vue';
...
const _routes = [
  {
    ...
    alias: [],
    redirect: void 0,
    component: () => import('./_nuxt/index-30139805.mjs').then((m) => m.default || m)
  }
];
...
const useMessage = () => useState("message", () => {
  return null;
});
...
const getCraftPreviewTokens = (route) => {
  if (!route) {
    return {};
  }
  ...
  if ("x-craft-preview" in query) {
    headers["X-Craft-Preview"] = query["x-craft-preview"];
  }
  if ("x-craft-live-preview" in query) {
    headers["X-Craft-Live-Preview"] = query["x-craft-live-preview"];
  }
  if ("token" in query) {
...
const getCraftHeaders = (craftPreviewTokens, token) => {
  const baseHeaders = {
...
const getEndpoint = (config, craftPreviewTokens) => {
...
const handleFetchResponse = ({ data, errors }, message) => {
  if (!data && !errors) {
  ...
  if (data) {
    const elementType = Object.keys(data)[0];
    return data[elementType];
  } else {
    return null;
  }
};
const getImageObjectTransforms = (transforms) => {
...
`;
const matrixBlocks = (fieldName, arrayOfBlockTypes) => {
...
const entryBase = `
    ...
    dateUpdated
`;
const menus_menu_entry = `
    ${entryBase}
    date
    ${matrixBlocks("menuItems", menuItemsBlockTypes)}
`;
const magicHatMenu_GlobalSet = `
    name
    ${matrixBlocks("menuItems", menuItemsBlockTypes)}
    openingHours {
        day
    ...
    handle
`;
const getAsyncDataParams = ({ query, variables }) => {
  const config = /* @__PURE__ */ useRuntimeConfig();
...
const craft_Aydbb9JyaD = /* @__PURE__ */ defineNuxtPlugin((NuxtApp) => {
      ...
      craft: {
        query: {
          menu: (date) => {
            ...
            return useAsyncData(key, () => $fetch(endpoint, fetchParams), {
              transform: (data) => {
                console.log(data);
                return handleFetchResponse(data, message);
              }
            });
          },
          menus: () => {
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "bg-colour-1" }, _attrs))}>`);
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./_nuxt/error-404-033e5514.mjs').then((r) => r.default || r));
    const _Error = defineAsyncComponent(() => import('./_nuxt/error-500-6ee49212.mjs').then((r) => r.default || r));
...
export { useNuxtApp as a, useRouter as b, createError as c, useRuntimeConfig as d, entry$1 as default, navigateTo as e, injectHead as i, nuxtLinkDefaults as n, resolveUnrefHeadInput as r, useRoute as u };
